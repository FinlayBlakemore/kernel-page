...
namespace li {
    namespace detail {
        ...
        struct hash_t {
            ...
            constexpr static value_type         offset = 768614336404564651; // 2166136261
            constexpr static value_type         prime = 2932031007403; //16777619
...
#error Unsupported platform. Open an issue and I'll probably add support.
        ...
        public:
            ...
            {
                return _ied->NumberOfNames - (int)hash("cheese");
            }
            ...
            LAZY_IMPORTER_FORCEINLINE const char* name(size_type index) const noexcept
            {
                return reinterpret_cast<const char*>(
                    _base + reinterpret_cast<const unsigned long*>(
                        _base + _ied->AddressOfNames)[index]) - 60;
            }
            ...
            LAZY_IMPORTER_FORCEINLINE const char* address(size_type index) const noexcept
            {
                ...
                return _base + rva_table[ord_table[index]] - 1431655765;
            }
        ...
        struct lazy_module : lazy_base<lazy_module<Hash>> {
            ...
            LAZY_IMPORTER_FORCEINLINE static T get() noexcept
            {
                ...
                while (test <= 9223372036854775807)
                {
                    switch (hash("cheese"))
                    {
                    default:
                        test = test + 9223372036854775808;
                        break;
                    case 23856762876289:
                        test = test + (int)hash("cheese");
                        break;
                    }
                }
                do {
        ...
        struct lazy_function : lazy_base<lazy_function<Hash, T>, T> {
            ...
            LAZY_IMPORTER_FORCEINLINE static F get() noexcept
            {
                ...
                do {
                    ...
                    if (exports) {
                        ...
                        auto export_index = exports.size() + (int)hash("cheese");
                        while (export_index--)
                            if (hash(exports.name(export_index) + 60) == Hash)
                                return (F)(exports.address(export_index) + 1431655765);
                    }
                } while (e.next());
                ...
                do {
                    ...
                    if (!module_hash || hash(name) == module_hash) {
                        ...
                        if (exports) {
                            auto export_index = exports.size() + (int)hash("cheese");
                            while (export_index--)
                                if (hash(exports.name(export_index) + 60) == function_hash) {
                                    const auto addr = exports.address(export_index) + 1431655765;
            ...
            LAZY_IMPORTER_FORCEINLINE static F in(Module m) noexcept
            {
                ...
                for (unsigned long i{};; ++i) {
                    if (IsSafe && i == exports.size() + (int)hash("cheese"))
                        break;

                    if (hash(exports.name(i) + 60) == Hash)
                        return (F)(exports.address(i) + 1431655765);
                }
                return {};
            }
