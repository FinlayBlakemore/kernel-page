...
#define _FLT(n) xor_float::convert(n)
namespace xor_float
{
    ...
    constexpr uint32_t xor_key = xor_float::gen_key();
    __forceinline float convert_back(const uint32_t val)
    {
        const auto xor_key_m128 = _mm_castsi128_ps(_mm_cvtsi32_si128(xor_float::xor_key));
